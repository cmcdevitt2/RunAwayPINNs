...
class EnergyTransformLayer(torch.nn.Module):
    """Layer that adds 0.511*(gamma-1) transformation to the first input"""
    def __init__(self):
        super().__init__()
        
    def forward(self, inputs):
        # inputs shape: (batch_size, 6) where 6 = (pNorm, xiNorm, tNorm, EFNorm, ZeffNorm, alphaNorm)
        pNorm = inputs[:, 0:1]  # First input (p normalized)
        
        # Convert normalized p back to physical p
        p = (pMax - pMin) * pNorm + pMin
        
        # Calculate gamma = sqrt(1 + p^2)
        gamma = torch.sqrt(1 + p * p)
        
        # Calculate 0.511*(gamma-1) - this is the energy in MeV
        energy_transform = 0.511 * (gamma - 1)
        
        # Concatenate original inputs with the energy transformation
        # Output shape: (batch_size, 7) where 7 = (pNorm, xiNorm, tNorm, EFNorm, ZeffNorm, alphaNorm, energy_transform)
        return torch.cat([inputs, energy_transform], dim=1)

class FCNN(torch.nn.Module):
    def __init__(self, input_dim, features):
        super().__init__()
        self.energy_transform = EnergyTransformLayer()
        
        layers = []
        # First layer now takes input_dim + 1 (original inputs + energy transformation)
        for i, fs in enumerate(features):
            in_dim = (input_dim + 1) if i == 0 else features[i-1]
    ...
    def forward(self, inputs):
        transformed_inputs = self.energy_transform(inputs)
        return self.net(transformed_inputs)
def output_transform_6D(inputs, outputs):
    ...
    Prob = pNorm*outputs[:,0:1]**2
    return Prob

def plot_energy_6D_comprehensive(model, EF_values, Zeff_values, alpha_values):
    """Create comprehensive energy vs time plots varying over EF, Zeff, and alpha parameters"""
...
def create_EF_variation_plot(model, EF_values, Zeff_values, alpha_values,
                            pgrid, pgridreal, g_array, xigrid, xigrid_real,
                            tgrid, tgrid_real, f_p_RE, colors):
    """Plot energy vs time for varying EF values, fixed Zeff and alpha"""
    ...
    for i, EF_val in enumerate(EF_values):
        ...
        # Calculate energy
        case = 'isotropic'
        energy = calculate_energy(y_pred_transformed, pgridreal, g_array, 
                                  xigrid_real, f_p_RE,case)
        
        ax.plot(tgrid_real, energy, color=colors[i % len(colors)], 
    ...
    ax.set_ylabel('Average Energy [MeV]', fontsize=22)
    title_str=f'$Z_{{eff}}={fixed_Zeff}$, $\\alpha={fixed_alpha}$'
    ...
    fig.savefig(f'{results_dir}/energy_vs_time_EF_variation_{case}.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def create_Zeff_variation_plot(model, EF_values, Zeff_values, alpha_values,
                              pgrid, pgridreal, g_array, xigrid, xigrid_real,
                              tgrid, tgrid_real, f_p_RE, colors):
    """Plot energy vs time for varying Zeff values, fixed EF and alpha"""
    # Use middle values for fixed parameters
    ...
    ax.set_ylabel('Average Energy [MeV]', fontsize=22)
    title_str = f'$E_\Vert={fixed_EF}$, $\\alpha={fixed_alpha}$'
    ax.set_title(title_str, fontsize=22)
    ax.legend(loc='best', fontsize=22)
    ax.grid(True, alpha=0.3)
    ax.set_xlim(0, 20)
    ...
    results_dir = f'{dataout}/results'
    os.makedirs(results_dir, exist_ok=True)
    fig.savefig(f'{results_dir}/energy_vs_time_Zeff_variation_{case}.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def create_alpha_variation_plot(model, EF_values, Zeff_values, alpha_values,
                               pgrid, pgridreal, g_array, xigrid, xigrid_real,
                               tgrid, tgrid_real, f_p_RE, colors):
    """Plot energy vs time for varying alpha values, fixed EF and Zeff"""
    # Use middle values for fixed parameters
    fixed_EF = EF_values[len(EF_values)//2]
    fixed_Zeff = Zeff_values[len(Zeff_values)//2]
    ...
    ax.set_xlabel(r'$t/\tau_c$', fontsize=22)
    ax.set_ylabel('Average Energy [MeV]', fontsize=22)
    title_str = f'$E_\Vert={fixed_EF}$, $Z_{{eff}}={fixed_Zeff}$'
    ax.set_title(title_str, fontsize=22)
    ax.legend(loc='best', fontsize=22)
    ax.grid(True, alpha=0.3)
    ax.set_xlim(0, 20)
    ...
    results_dir = f'{dataout}/results'
    os.makedirs(results_dir, exist_ok=True)
    fig.savefig(f'{results_dir}/energy_vs_time_alpha_variation_{case}.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def load_jonta_data(EF_val, Zeff_val, alpha_val, orientation='isotropic'):
    ...
    """
    try:
        jonta_dir = f"REmoments_E={"%.2f"%EF_val}_Zeff={"%.2f"%Zeff_val}_alpha={"%.4f"%alpha_val}_{orientation}/data/E_vs_time.txt"
...
def load_jonta_data_full(EF_val, Zeff_val, alpha_val, orientation='isotropic'):
    ...
    """
    try:
        jonta_dir = f"REmoments_E={"%.2f"%EF_val}_Zeff={"%.2f"%Zeff_val}_alpha={"%.4f"%alpha_val}_{orientation}_full/data/E_vs_time.txt"
        X, y = np.loadtxt(jonta_dir).T
        return X/1000, y  # Convert time units
    except FileNotFoundError:
        print(f"Warning: Jonta data not found for EF={EF_val}, Zeff={Zeff_val}, alpha={alpha_val}, orientation={orientation}")
        return None, None
def calculate_energy(y_pred_transformed, pgridreal, g_array, xigrid_real, f_p_RE,case):
    """Calculate energy by integrating over momentum and pitch angle"""
    nump = len(pgridreal)
...
def plot_orientation_comparison(model, EF_val, Zeff_val, alpha_val,
    ...
    """Plot energy vs time for different orientations at fixed E, Zeff, alpha"""
    ...
    for i, case in enumerate(orientations):
        ...
        ax.plot(tgrid_real, energy, color=colors[i], 
    ...
    results_dir = f'{dataout}/results'
    os.makedirs(results_dir, exist_ok=True)
    fig.savefig(f'{results_dir}/energy_vs_time_orientation_comparison_E{EF_val}_Zeff{Zeff_val}_alpha{alpha_val}.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def main():
    ...
    plot_energy_6D_comprehensive(model, EF_values, Zeff_values, alpha_values)
