...
def input_transform_6D(inputs):
    """Input transform function that adds 0.5*(3*xi^2-1) as an additional feature"""
    pNorm, xiNorm, tNorm, EFNorm, ZeffNorm, alphaNorm = inputs[:, 0:1], inputs[:, 1:2], inputs[:, 2:3], inputs[:, 3:4], inputs[:, 4:5], inputs[:, 5:6]
    # Convert xi from normalized [0, 1] to physical [-1, 1]
    p = (pMax-pMin)*pNorm + pMin
    xi = (xiMax-xiMin)*xiNorm+xiMin

    gamma = torch.sqrt(p**2+1)
    energy = 0.511*(gamma-1)
    # Concatenate original inputs with the new feature
    transformed_inputs = torch.cat([inputs, energy], dim=1)
...
class FCNN(torch.nn.Module):
    ...
    def forward(self, inputs):
        transformed_inputs = input_transform_6D(inputs)
...
def tMin_func_6D(X):
    ...
    Pinit = 0.511*(gamma-1)* 0.5 * (1.0 - torch.tanh(Prob))
    return Pinit
...
def plot_EPF_6D(model, Zeffgrid, alphagrid, tgrid, EFgrid, losses, domain_points):
    """Plot RPF for 6D problem - now with Zeff and alpha"""
    ...
    fig0.savefig(f'{plots_dir}/EPF_6D_tgrid={tgrid_str}_EF={EF_str}_Zeff={Zeff_str}_alpha={alpha_str}_pytorch.png', dpi=300, bbox_inches='tight')
    fig1.savefig(f'{plots_dir}/EPF_6D_tgrid={tgrid_str}_EF={EF_str}_Zeff={Zeff_str}_alpha={alpha_str}_pytorch_res.png', dpi=300, bbox_inches='tight')
    plt.close(fig0)
    
    print(f"Plots saved: RPF_6D_tgrid={tgrid_str}_EF={EF_str}_Zeff={Zeff_str}_alpha={alpha_str}.png")
    return
        ...
        if epoch % args.adaptive_freq == 0:
            ...
            for EF_val in [2.0]:
                ...
                for Zeff_val in [2.0]:
                    ...
                    for alpha_val in [alphaMin, alphaMax]:
                        alpha_norm = (alpha_val - alphaMin) / (alphaMax - alphaMin)
                        plot_RPF_6D(model, Zeffgrid=Zeff_norm, alphagrid=alpha_norm, tgrid=tMin, EFgrid=EF_norm, losses=None, domain_points=boundary_points)
                        plot_RPF_6D(model, Zeffgrid=Zeff_norm, alphagrid=alpha_norm, tgrid=0.5, EFgrid=EF_norm, losses=None, domain_points=domain_points)
...
def main():
    ...
    if args.train:
        ...
        for tval in [0.0,0.5,1.0]:
            plot_RPF_6D(model, Zeffgrid = (3-ZeffMin)/(ZeffMax-ZeffMin), alphagrid= (0.1-alphaMin)/(alphaMax-alphaMin),tgrid=tval,EFgrid = (2.5-EFMin)/(EFMax-EFMin),losses=None,domain_points=None)
