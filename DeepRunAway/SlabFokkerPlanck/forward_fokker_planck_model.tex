...
electron Fokker--Planck model and its GPU finite-volume realization.  Standalone mode
...
state.  An optional bulk-plasma driver is implemented and exchanges stage plasma
states and kinetic current moments while preserving the conservative finite-volume
and direct-solver contracts defined here.
...
and holds them constant in time.  The optional coupled driver evolves a host-side bulk
state and supplies stage plasma states and kinetic current moments; spatial transport
remains outside the current model.  The physical system is defined below in dimensional
...
\begin{equation}
 w^p_{i+1/2,j}=\frac{A^p_{i+1/2,j}\Delta p}{C_{A,i+1/2}}.
...
and the face Peclet argument
\begin{equation}
 w^\xi_{i,j+1/2}=\frac{A^\xi_{i,j+1/2}\Delta\xi}{D^\xi_{i,j+1/2}}.
...
\subsection{Fully implicit TR--BDF2 time stepping}
...
The implemented two-stage TR--BDF2 method uses
\begin{equation}
...
stage matrix; no separate startup scheme is used.
...
trial step changes, the numerical matrix values are reassembled and cuDSS is
refactorized while its analyzed CSR topology is reused.
...
In coupled mode, each TR--BDF2 stage obtains a host-side bulk state and kinetic current
guess.  The kinetic coefficient values and Chiu--Harvey stage map are rebuilt from that
state, while the fixed CSR topology is retained and numerical values are refactorized.
The coupled stage fixed-point iteration must converge before the timestep is accepted.
Implicit treatment removes explicit stability restrictions associated with stiff collision
and advection terms, but not the timestep requirement for accuracy.  Cell contents are
monitored for loss of non-negativity.
...
\begin{enumerate}
 ...
 \item assemble the common TR--BDF2 stage matrix;
 \item perform one sparse symbolic analysis of that fixed graph;
 \item numerically factor the stage matrix and solve the intermediate TR stage;
 \item solve the endpoint TR--BDF2 stage with the retained factorization;
 \item in adaptive standalone mode, reassemble and refactor numerical values when the
       trial timestep changes while reusing the analyzed topology;
 \item in coupled mode, update numerical values for each bulk stage and refactorize while
       reusing the analyzed topology.
\end{enumerate}
Each accepted fixed-step requires two sparse direct solves.  An adaptive step also uses
the embedded error-estimate solve.  Coefficient or timestep changes require numerical
refactorization but not CSR symbolic reanalysis when topology remains unchanged.
...
\begin{enumerate}
 ...
 \item \textbf{Spatial convergence of the kinetic equation.}
       ...
       respect to the first-order drift discretization as $\Delta p$ and $\Delta\xi$ are independently
...
%===============================================================================
\section{Implemented bulk-plasma coupling and remaining qualification}
\label{sec:bulk_coupling}

Standalone mode remains the reference calculation: it advances the kinetic distribution
using a prescribed, constant plasma state.  The code also implements an optional coupled
GPU driver.  Its nonlinear bulk work is host-side and its kinetic state, CSR graph,
numerical values, and cuDSS vectors remain on one CUDA device.
...
\begin{itemize}
  ...
  \item each accepted kinetic timestep performs TR--BDF2 intermediate and endpoint
        bulk stages.  Kinetic current moments feed the bulk stages, and a fixed-point
        iteration continues until the configured scaled stage residual converges;
  \item each bulk stage rebuilds host coefficient arrays and GPU CSR numerical values,
        while the fixed mesh topology and cuDSS symbolic analysis are retained;
  ...
  \item local OpenADAS ADF11 bundles and authoritative state-resolved excitation and
        screening data are required for each coupled species.  Coupled restart output
        stores the kinetic state, bulk temperature, charge populations, current, time,
        configuration, solver metadata, and OpenADAS provenance.
\end{itemize}

Coupling remains implemented but not fully qualified.  Required qualification includes
comparison against a prescribed-state run with identical coefficients, particle/energy/
current conservation across the exchange, coefficient parity over representative bulk
states, topology reuse versus full rebuilds, restart equivalence, and timestep, mesh, and
coupling-cadence refinement.  Until these checks are complete, standalone forward
evolution remains the reference calculation.
