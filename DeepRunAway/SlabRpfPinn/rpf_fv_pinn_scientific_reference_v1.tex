...
\begin{itemize}[leftmargin=2em]
 ...
 \item electric acceleration, relativistic small-angle collisions, synchrotron reaction,
       and the associated open radial boundaries;
...
\end{equation}
with $p_{\min}>0$ for the present suprathermal first-passage problem.
...
The coefficient construction is shared with the qualified kinetic model and may include
the selected finite-temperature relativistic base, energy-dependent Coulomb logarithms,
and partial-screening corrections.  No separate adjoint collision model is introduced.
...
Radial and pitch drifts use first-order upwinding; diffusion uses centered differences.
...
\end{equation}
The pitch-face flux has the analogous upwind/centered structure with $A_\xi$ and $D_\xi$.
...
\end{equation}
where, for the first-order outer flux,
\begin{equation}
 (r_{\rm RE})_{ij}
 =\begin{cases}
 0, & i<N_p,\\[2mm]
