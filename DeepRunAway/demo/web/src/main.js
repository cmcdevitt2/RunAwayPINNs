...
function cleanPlotCards() {
  hideSingleElementOnly("probColorbar");
  hideSingleElementOnly("resColorbar");
...
function drawInlineColorbar(ctx, spec) {
  const steps = Math.max(1, Math.round(spec.height));

  for (let j = 0; j < steps; j += 1) {
    const v = 1.0 - j / Math.max(steps - 1, 1);
    const [r, g, b] = spec.cmap(v);

    ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
    ctx.fillRect(spec.x, spec.y + j, spec.width, 1.25);
  }
  ...
  ctx.fillStyle = spec.topTextColor;
  ctx.fillText(spec.maxText, spec.x + 8, spec.y + 10);

  ctx.textBaseline = "bottom";
  ctx.fillStyle = "white";
  ctx.fillText("0", spec.x + 8, spec.y + spec.height - 10);
