...
Features:
  - E_OX high-energy BC gate using OXmerger.py if available
  ...
  - reduced O/X null-curve plotting with U_p=0 and scattering-inclusive
    Gamma_xi=0
  - optional low-level Optimistix SSB driver that records per-step SSB losses
...
import optax
from optimistix._misc import NoneAux, OutAsArray, inexact_asarray, tree_full_like, tree_where
...
SSB_VERBOSE = False
SSB_RECORD_INTERNAL_LOSS = True
SSB_TEST_EVERY_BLOCKS = 1
SSB_PRINT_EVERY_BLOCKS = 1
...
def sample_high_boundary(key, n):
    ...
    xi_hi = jnp.clip(xi_crit - jnp.asarray(1.0e-10, dtype=jnp.float64), XI_MIN, XI_MAX)
...
def _residual_abs_chunk(params, x_chunk):
    return jnp.abs(residual_apply(params, x_chunk)[:, 0])
...
def train_ssbroyden(params, key):
    ...
    print(f"SSB internal loss history: {SSB_RECORD_INTERNAL_LOSS}")

    history = []

    def scalar_loss(w, batch):
        xpde, xhi = batch
    ...
    @eqx.filter_jit
    def ssb_solve_with_history(w0, xpde, xhi):
        """Run one SSB block using Optimistix's low-level API.

        This keeps the actual Optimistix SSB implementation, but replaces the
        optx.minimise convenience driver with a local lax.scan so we can return
        one scalar training loss per internal SSB iteration.
        """
        batch = (xpde, xhi)
        options = {}
        tags = frozenset()

        w0 = jtu.tree_map(inexact_asarray, w0)
        fn = NoneAux(scalar_loss)
        fn = OutAsArray(fn)
        fn = eqx.filter_closure_convert(fn, w0, batch)
        f_struct, aux_struct = fn.out_struct

        init_aux = tree_full_like(aux_struct, 0)
        init_state = solver.init(fn, w0, batch, options, f_struct, aux_struct, tags)
        dynamic_state, static_state = eqx.partition(init_state, eqx.is_array)

        loss_before = scalar_loss(w0, batch)

        def scan_body(carry, _):
            w, dynamic_state_i, aux_i, done_i, num_steps_i, last_loss_i = carry
            state_i = eqx.combine(static_state, dynamic_state_i)
            terminate_i, result_i = solver.terminate(fn, w, batch, options, state_i, tags)
            active_i = jnp.logical_not(done_i | terminate_i)

            def do_step(_):
                state_step = eqx.combine(static_state, dynamic_state_i)
                w_new, state_new, aux_new = solver.step(fn, w, batch, options, state_step, tags)
                dynamic_state_new, _ = eqx.partition(state_new, eqx.is_array)
                loss_new = scalar_loss(w_new, batch)
                return w_new, dynamic_state_new, aux_new, loss_new

            def no_step(_):
                return w, dynamic_state_i, aux_i, last_loss_i

            w_new, dynamic_state_new, aux_new, loss_new = jax.lax.cond(active_i, do_step, no_step, operand=None)
            done_new = done_i | terminate_i
            num_steps_new = num_steps_i + active_i.astype(jnp.int32)
            return (w_new, dynamic_state_new, aux_new, done_new, num_steps_new, loss_new), (loss_new, active_i)
        ...
        final_carry, scan_out = jax.lax.scan(scan_body, init_carry, xs=None, length=SSB_BLOCK_ITERS)
        w_last, dynamic_state_final, aux_last, done_final, num_steps, last_loss = final_carry
        losses, active = scan_out

        final_state = eqx.combine(static_state, dynamic_state_final)
        terminate_final, result = solver.terminate(fn, w_last, batch, options, final_state, tags)
        result = optx.RESULTS.where(
            (result == optx.RESULTS.successful) & jnp.logical_not(terminate_final),
            optx.RESULTS.nonlinear_max_steps_reached,
            result,
        )
        w_final, aux_final, stats = solver.postprocess(fn, w_last, aux_last, batch, options, final_state, tags, result)
        loss_after = scalar_loss(w_final, batch)
        delta = jnp.linalg.norm(w_final - w0)
        finite = jnp.all(jnp.isfinite(w_final)) & jnp.isfinite(loss_after)
        return w_final, loss_before, loss_after, delta, finite, num_steps, losses, active

    @jax.jit
    def ssb_solve_minimise(w0, xpde, xhi):
        batch = (xpde, xhi)
        loss_before = scalar_loss(w0, batch)
        sol = optx.minimise(scalar_loss, solver, w0, args=batch, max_steps=SSB_BLOCK_ITERS, throw=False)
        w_final = sol.value
        loss_after = scalar_loss(w_final, batch)
        delta = jnp.linalg.norm(w_final - w0)
        finite = jnp.all(jnp.isfinite(w_final)) & jnp.isfinite(loss_after)
        num_steps = sol.stats["num_steps"]
        losses = jnp.full((SSB_BLOCK_ITERS,), jnp.nan, dtype=w0.dtype)
        active = jnp.zeros((SSB_BLOCK_ITERS,), dtype=bool)
        return w_final, loss_before, loss_after, delta, finite, num_steps, losses, active
    ...
    for block in range(1, SSB_BLOCKS + 1):
        ...
        if SSB_RECORD_INTERNAL_LOSS:
            out = ssb_solve_with_history(weights, x_pde, x_high)
        else:
            out = ssb_solve_minimise(weights, x_pde, x_high)

        weights_new, loss_before, loss_after, delta, finite, num_steps, losses, active = out
        jax.block_until_ready(weights_new)

        test_total = eval_total_flat(weights_new, x_test, x_test_high)
        train_total = eval_total_flat(weights_new, x_pde, x_high)
        (
            weights_new,
            loss_before,
            loss_after,
        ...
        block_start_iter = int(ADAM_STEPS + ssb_iter)

        # Record the block-start loss so resampling jumps are visible on the plot.
        history.append(
            {
                "phase": "ssbroyden_start",
                "iter": block_start_iter,
                "step": int(block),
                "train_total": float(loss_before),
                "test_total": float("nan"),
                "block_loss_before": float(loss_before),
                "block_loss_after": float("nan"),
                "block_delta_weights": float("nan"),
                "block_num_steps": 0,
            }
        )

        if SSB_RECORD_INTERNAL_LOSS and num_steps_int > 0:
            losses_np = np.asarray(losses)
            active_np = np.asarray(active).astype(bool)
            for j in range(min(num_steps_int, losses_np.shape[0])):
                if active_np[j]:
                    history.append(
                        {
                            "phase": "ssbroyden",
                            "iter": int(block_start_iter + j + 1),
                            "step": int(block),
                            "train_total": float(losses_np[j]),
                            "test_total": float("nan"),
                        }
                    )
        else:
            history.append(
                {
                    "phase": "ssbroyden",
                    "iter": int(block_start_iter + num_steps_int),
                    "step": int(block),
                    "train_total": float(loss_after),
                    "test_total": float("nan"),
                }
            )

        ssb_iter += num_steps_int
        weights = weights_new

        do_test = block == SSB_BLOCKS or (SSB_TEST_EVERY_BLOCKS > 0 and block % SSB_TEST_EVERY_BLOCKS == 0)
        if do_test:
            history.append(
                {
                    "phase": "ssbroyden_test",
                    "iter": int(ADAM_STEPS + ssb_iter),
                    "step": int(block),
                    ...
                    "block_num_steps": int(num_steps_int),
                }
            )

        do_print = block == SSB_BLOCKS or (SSB_PRINT_EVERY_BLOCKS > 0 and block % SSB_PRINT_EVERY_BLOCKS == 0)
        if do_print:
            print(
                f"ssb block {block:4d}/{SSB_BLOCKS} "
                f"block={float(loss_before):.3e}->{float(loss_after):.3e} "
                f"steps={num_steps_int} finite={bool(finite)} |dw|={float(delta):.3e} "
                f"train={float(train_total):.3e} test={float(test_total):.3e}"
            )
